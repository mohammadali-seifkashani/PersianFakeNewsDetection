...
FaradadeDWConn = create_engine(
    "mssql+pyodbc://dangerous_seif:%s@192.168.5.221:1433/FaradadehDW?driver=ODBC+Driver+17+for+SQL+Server&charset=utf8" \
    % quote_plus("2520")
)
...
def DeleteWithCondtion(tablename: str, con, column, value, schema: str = 'dbo') -> None:
    query = text(f"DELETE FROM {schema}.{tablename} WHERE {column} = '{value}'")
...
def persian_to_utc(persian_str):
    persian_dt = jdatetime.datetime.strptime(persian_str, "%Y-%m-%d %H:%M:%S")
...
class ElasticUtils:
    ...
    def remove_embeddings(self, index_name='daily_trends_posts', days=10):
        print(f'REMOVING EMBEDDINGS OF BEFORE {days} DAYS AGO:')
...
es = Elasticsearch("http://192.168.5.220:9200")
EU = ElasticUtils(es)


if __name__ == '__main__':
    s = "1401-09-27 21:33:12"
    print(per(s))