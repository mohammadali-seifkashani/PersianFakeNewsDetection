...
from mymaincodes.PackDataset import packDataset_util_bert_with_all
...
class BERTModelExtrFeaturesWithLinear(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, additional_input1, additional_input2, additional_input3,
                additional_input4, additional_input5):
        ...
        combined_output = torch.cat(
            (linear_output, additional_input1, additional_input2, additional_input3, additional_input4, additional_input5), dim=1)
        logits = self.classifier(combined_output)
        logit_softmax = self.softmax(logits / self.temperature)
        return logit_softmax
    

def normalize_data(data, dindex):
    length = len(data[0][dindex])
    min_ds = [np.inf] * length
    max_ds = [-np.inf] * length

    for d in data:
        part = d[dindex]
        for j in range(-length, 0, 1):
            min_ds[j] = min(min_ds[j], part[j])
            max_ds[j] = max(max_ds[j], part[j])
    
    for d in data:
        for j in range(-length, 0, 1):
            d[dindex][j] = (d[dindex][j] - min_ds[j]) / (max_ds[j] - min_ds[j])
...
def get_train_dev_test_loaders(class1rate=2.5):
    ...
    if normalize_diffs_bool:
        normalize_data(data, -2)
        normalize_data(data, 0)
    # data = select_data(data, class1rate)
    # for d in data:
    #     avg = float(np.mean(d[0]))
    #     std = float(np.std(d[0]))
    #     s = sum(d[0])
    #     d[0] = (np.array(d[0]) / np.sum(d[0])).tolist() + [avg, std, s]
...
def train(model, criterion, optimizer, scheduler, train_loader_poison, early_stopping_rounds=3):
    ...
    try:
        for epoch in range(warm_up_epochs + EPOCHS):
            ...
            for padded_text, attention_masks, uem, categories, sentiments, tags, difficulity, labels in train_loader_poison:
                uem = uem.to(device)
                ...
                output = model(padded_text, attention_masks, uem, categories, sentiments, tags, difficulity)
                try:
...
def evaluation(loader):
    ...
    with torch.no_grad():
        for padded_text, attention_masks, uem, categories, sentiments, tags, difficulity, labels in loader:
            uem = uem.to(device)
            padded_text = padded_text.to(device)
            attention_masks = attention_masks.to(device)
            categories = categories.to(device)
            ...
            output = model(padded_text, attention_masks, uem, categories, sentiments, tags, difficulity)  # batch_size, 2
...
EPOCHS = 15
warm_up_epochs = 3
additional_features = 10 + len(categories) + len(sentiments) + len(entities) + len(difficulty_H_M)
...
model.load_state_dict(torch.load("user14040206.pth", map_location=device))
...
# torch.save(model.state_dict(), "user14040206.pth")
