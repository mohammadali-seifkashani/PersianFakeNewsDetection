...
class processed_dataset_bert_with_all(Dataset):
    def __init__(self, data):
        ...
        for uem, text, category, sentiment, tag, difficulity, label in data:
            self.userembeddings.append(uem)
            self.texts.append(torch.tensor(tokenizer.encode(text, max_length=128, truncation=True)))
            self.categories.append(category)
    ...
    def __getitem__(self, idx):
        return self.userembeddings[idx], self.texts[idx], self.categories[idx], self.sentiments[idx], self.tags[idx], self.difficulities[idx], self.labels[idx]
...
class packDataset_util_bert_with_all:
    ...
    def fn(self, data):
        ...
        labels = torch.tensor(labels)
        userembeddings = torch.tensor(userembeddings)
        categories = torch.tensor(categories)
        ...
        padded_texts = pad_sequence(texts, batch_first=True, padding_value=0)
        attention_masks = torch.zeros_like(padded_texts).masked_fill(padded_texts != 0, 1)
        return userembeddings, padded_texts, attention_masks, categories, sentiments, tags, difficulities, labels
